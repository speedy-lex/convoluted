...
use convoluted::{array::Array1D, cost::CrossEntropy, layer::{convolution::Convolution, dense::DenseLayer, relu::ReluLayer, reshape::{Flatten, Shape}, sigmoid::SigmoidLayer, Layer, LayerChain}};
...
fn main() {
    let layer = Network::load("network.bin").unwrap().into_layer().step.step.step.step;
    let train = mnist::get_mnist_train().0.into_iter().map(|x| {
        layer.forward(x).0
    }).collect::<Vec<_>>();
    let x = layer.step.step;
    let conv = x.next.kernel;
    let conv2 = x.step.step.step.step.next.kernel;
    println!("{:?}\n{:?}", conv.array, conv2.array);
    ...
    while !rl.window_should_close() {
        ...
        for y in 0..28 {
            for x in 0..28 {
                let col = (train[idx][x + 28*y] * 256.0).floor();
                let col = if col < 0.0 {
